...
<p align="center">
  <img src="public/favicon.svg" alt="Shelt — 喜乐蒂牧羊犬" width="160" height="160" />
</p>
...
- 终端中的绝对路径可点击，预览 Markdown、交互 HTML、SVG 和图片
...
登录后，在文档预览页右上角点击「分享」→「创建并复制链接」。接收者可通过独立的 `/share/<key>` 链接匿名阅读当前文件内容及受控的本地引用图片，不会获得终端、上传或其他文档权限。链接默认有效 7 天，可在原预览页撤销；重新生成会立即使旧链接失效。

服务端只保存 key 的 SHA-256 哈希，记录位于状态目录的 `shares.json`，重启后仍有效。离开页面后不能找回原始链接，只能重新生成。分享页不提供在线朗读或分享管理；HTML/SVG 保持静态沙箱预览。请仅向可信任的人发送链接，并使用 HTTPS；撤销无法收回接收者已经保存的内容。

## 连接与密码设置

登录后，终端页右上角提供设置按钮。在 Herdr 模式下，可以维护一组 Herdr 连接（本机 + 多个 `herdr --remote` SSH 目标，可选命名会话），点击列表项即切换：服务端以参数数组直接启动 `herdr --remote <目标> [--session <会话>]`，不经过 shell，也无需在已运行的 Herdr 里嵌套启动。配置保存在状态目录的 `herdr-targets.json`（私有权限），重启后保留当前选择。shell 模式下连接列表自动隐藏。

新增连接的表单默认收起，点击连接标题旁的「＋」展开；编辑现有连接也会展开同一表单。修改密码默认收起，点击「修改密码」展开，保存成功或取消后再次收起；验证当前密码后写入新的 Argon2id 哈希，其他浏览器的会话立即失效，当前浏览器保持登录。

设置菜单和预览页工具条提供「中文 / English」切换，默认中文，按浏览器保存并同步到同源标签页。切换只改变界面文案，不翻译文档正文、连接名称或 Herdr 终端内容，也不会重新加载预览或中断朗读。

悬停设置按钮可触发 Sheltie 探头：每次在按钮左侧至下侧范围内随机选择方向、距离和倾斜角度，连续两次不会选到同一方向区间。自动出现仍为随机 3–5 分钟；系统开启「减少动态效果」时只保留主动悬停触发，不自动播放。

## HTML 交互预览

设置中的「HTML 交互预览」默认开启，按当前浏览器保存；更改后，同一浏览器已打开的 HTML 预览会立即切换。开启时支持内联 JavaScript、事件处理、ES 模块、Canvas/WebGL、外部脚本和样式，以及当前 HTML 所在目录及子目录中的 JS、CSS、图片、字体、JSON 等网页资源。相对资源 URL 保留目录结构，不能访问上级目录、隐藏文件或 Shelt 状态目录，也不提供目录列表。请将需要预览的网页及资源放在独立目录内，不要混放私密数据。

交互页面运行在不具有 Shelt 同源权限的沙箱中，不能读取父页面、登录存储或操作终端/设置。资源地址为随机临时凭据，只关联当前登录会话，最长有效 24 小时；切回静态模式或正常关闭预览时会请求撤销，退出登录或服务重启后失效。浏览器异常退出时，地址可能保留至会话失效或到期。不要把临时资源地址当成分享链接。页面可访问外部网络，外部服务仍受网络可达性、浏览器跨域和混合内容规则限制。

关闭开关后恢复原来的静态安全预览（不执行脚本）。匿名分享、直接文件 API 和独立 SVG 预览始终保持静态。交互模式不支持朗读，会显示提示；切回静态模式可恢复朗读。依赖 `localStorage`、Service Worker、顶层跳转、弹窗或后端服务的完整网站不能保证兼容；根路径 `/assets/...` 不会映射到磁盘目录，请使用相对路径。
